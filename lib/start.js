...
const daftar1 = `*${jams}* \n*Sebelum Mengakses Bot Wajib*\n*Verifikasi Terlebih Dahulu*`
const daftar2 = '© - RyGiiXyzzz'
...
const jancok =`*[ Berhasil Verifikasi ]*
*Nama :* *${pushname}*
...
td =`❏ *PLAY MP3*\n*JUDUL / TITLE :* ${get.title}`
tdd =`© - RyGiiXyzzz`
but = [
{buttonId:`${prefix}mp3 ${args.join(" ")}`,buttonText:{displayText: '🎧 Music'},type:1},
{buttonId:`${prefix}mp4 ${args.join(" ")}`,buttonText:{displayText: '📽️ Vodeo'},type:1} 
]
...
break
case 'nulis':
if (args.length == 0) return reply(`Text Nya Mana?\nContoh : ${prefix + command} Naruto Hacker`)
qo = args.join(" ")
sticWait(from)
...
break
    case 'grup':
            if(!q) return reply('Cari Grup Apa?')
            hx.linkwa(q)
            .then(result => {
            let res = '*「 _LINK WA_ 」*\n\n'
            for (let i of result) {
...
break
    ...
    case 'tourl':
        ...
        sendButLocation(from, ttrth, '© - RyGiiXyzzz <3',truteh, but, {quoted: mek})
	        	break
		case 'dare':
        ...
        sendButLocation(from, der, '© - RyGiiXyzzz <3',todz, but, {quoted: mek})
       	   break
          case 'simi':
           case 'p':
           case 'bot':
         ...
         for (let _ of anu) {
         LexxyOFC.sendMessage(_.jid, bc, image, { caption: `[ Bot izin Broadcast ]\n\n${body.slice(4)}` })
         }
...
case 'owner':
if (!isRegistered) return sendButton(from, daftar1, daftar2, daftar3, flexx)
sendKontak(from, `${owner}`, `RyGiiXyzzz`)
break      
default:
